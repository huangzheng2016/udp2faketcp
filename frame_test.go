...
func TestFrameNoKey(t *testing.T) {
	payload := []byte("hello")
	frame := encodeFrame(nil, nil, 0, 7, frameData, payload)
	if len(frame) != frameFixedLen+len(payload) {
		t.Fatalf("frame length = %d, want %d", len(frame), frameFixedLen+len(payload))
	}
	typ, streamSeq, _, got, ok := decodeFrame(nil, frame)
	if !ok || typ != frameData || streamSeq != 7 || !bytes.Equal(got, payload) {
		t.Fatalf("decode = (%d, %d, %q, %v)", typ, streamSeq, got, ok)
	}
}
...
func TestFrameWithKey(t *testing.T) {
	...
	payload := []byte("hello")
	frame := encodeFrame(nil, key, 42, 7, frameData, payload)
	if len(frame) != frameHeadLen+len(payload) {
		t.Fatalf("frame length = %d, want %d", len(frame), frameHeadLen+len(payload))
	}
	typ, streamSeq, hmacSeq, got, ok := decodeFrame(key, frame)
	if !ok || typ != frameData || streamSeq != 7 || hmacSeq != 42 || !bytes.Equal(got, payload) {
		t.Fatalf("decode = (%d, %d, %d, %q, %v)", typ, streamSeq, hmacSeq, got, ok)
	}
}
...
func TestFrameTamper(t *testing.T) {
	...
	if _, _, _, _, ok := decodeFrame(deriveKey("other"), encodeFrame(nil, key, 1, 1, frameData, []byte("hi"))); ok {
		t.Fatal("frame accepted under wrong key")
	}
	if _, _, _, _, ok := decodeFrame(key, []byte{0, 1, 2}); ok {
