...
Every fake-TCP payload is a small frame: a 1-byte type (data / heartbeat / handshake) followed by the payload.
Both sides exchange a heartbeat every second; a connection with no data for `-ttl` seconds is torn down with a RST,
so the peer and middleboxes free their state immediately instead of timing out.

With `-k`, each frame additionally carries an 8-byte sequence number and a 16-byte truncated HMAC-SHA256 tag
(key derived from the passphrase with SHA-256). The server only forwards traffic from clients that completed a
handshake frame, so the tunnel is no longer an open relay, and a 4096-bit replay window rejects replayed frames.
Note: `-k` authenticates packets, it does not encrypt them.
...
and the UDP payload budget is smaller by the frame header (1 byte, or 25 bytes with `-k`).
For a 1500-byte link with WireGuard on top (its own overhead is 32 bytes for IPv4):

```
udp2faketcp -mtu = 1500 - 40 (IP+TCP) - 32 (WireGuard) = 1428, minus 25 when using -k
WireGuard MTU     = 1500 - 40 - (frame header)          = 1459, or 1435 with -k
```
